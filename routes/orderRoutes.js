...
const createOrderValidation = [
  ...
  body('shippingAddress.zip').matches(/^\d{5}$/).withMessage('ZIP code must be 5 digits'),
  ...
  body('billingAddress.zip').matches(/^\d{5}$/).withMessage('Billing ZIP code must be 5 digits'),
...
router.get(
  ...
  [
    ...
    query('dateFrom').optional().isISO8601().toDate().withMessage('Invalid dateFrom'),
    query('dateTo').optional().isISO8601().toDate().withMessage('Invalid dateTo'),
